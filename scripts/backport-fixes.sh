...
Options:
  --from <ref>       Source ref on origin: a branch (e.g. release/v0.6.0,
                     ...
                     workflow has deleted the source branch.
  --to <branch>      Target branch on origin. Required.
...
Behavior:
  1. Fetches origin (both branch and tag refs).
  2. Lists commits on the source ref that aren't already on origin/<to>,
...
# Fetch the target branch and try to fetch FROM as both a branch and a tag.
# Branches go to refs/remotes/origin/<from>; tags go to refs/tags/<from>.
# The two explicit refspecs make either accepted; whichever doesn't exist
# is silently ignored.
git fetch origin "$TO" --no-tags
git fetch origin --no-tags \
  "+refs/heads/$FROM:refs/remotes/origin/$FROM" 2>/dev/null \
  || true
git fetch origin --no-tags \
  "+refs/tags/$FROM:refs/tags/$FROM" 2>/dev/null \
  || true

# Resolve FROM to a ref that exists locally after fetch.
if git rev-parse --verify "refs/remotes/origin/$FROM" >/dev/null 2>&1; then
  from_ref="refs/remotes/origin/$FROM"
  from_pretty="origin/$FROM"
elif git rev-parse --verify "refs/tags/$FROM" >/dev/null 2>&1; then
  from_ref="refs/tags/$FROM"
  from_pretty="$FROM (tag)"
else
  echo "error: $FROM not found on origin (tried both branches and tags)" >&2
  exit 1
fi

if ! git rev-parse --verify "refs/remotes/origin/$TO" >/dev/null 2>&1; then
